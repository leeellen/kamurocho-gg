...
export async function generateMetadata({
  ...
  return {
    ...
    keywords: [
      ...
      data.game.name,
      `${data.game.name} 업적`,
      ach.missable ? "Missable achievement" : "업적 공략",
    ...
    <SiteShell locale={locale} section="games">
      <article className="mx-auto max-w-[900px] px-5 pb-20 pt-12 md:px-8">
        ...
        >
          {tracksProgress && (
            <div
              className={`-mt-2 mb-5 inline-flex items-center gap-2 rounded-full px-3 py-1.5 text-[16px] font-bold ${
                isUnlocked
          ...
          <div className="flex flex-wrap items-start gap-4 md:flex-nowrap md:gap-6">
            ...
            <div className="flex-1 min-w-0">
              ...
              <h1 className="font-display m-0 mt-4 text-[32px] font-extrabold leading-[1.1] tracking-[-0.02em] text-white md:text-[40px]">
                {ach.name}
              </h1>
              <p className="m-0 mt-3 text-[16px] leading-7 text-[var(--text-secondary)] md:text-[17px]">
                {ach.description}
              </p>
            </div>
          </div>
          <div className="mt-5 flex items-center gap-3 border-t border-[var(--border-subtle)] pt-5">
            <span
              className={`font-mono text-[16px] text-[var(--text-tertiary)] ${
          ...
          {!user && (
            <div className="mt-5 flex flex-wrap items-center justify-between gap-4 rounded-xl border border-dashed border-[var(--border)] bg-[var(--bg-elevated)]/60 p-3 text-[16px]">
            ...
            <ol className="mt-5 flex flex-col gap-3" role="list">
              {ach.guideSteps.map((step, index) => (
                ...
                >
                  <span
                    aria-hidden="true"
                    className="flex h-7 w-7 shrink-0 items-center justify-center rounded-full bg-[var(--accent)] font-mono text-[16px] font-bold text-white"
                  >
                    {index + 1}
                  </span>
