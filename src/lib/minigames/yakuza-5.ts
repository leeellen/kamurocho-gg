  ...
  minigames: [
    {
      ...
      howTo: [
        ...
        { ko: "터보는 절대 코너에서 쓰지 말고 마지막 직선 구간을 위해 아껴두세요. 코너에서 터보를 쓰면 벽에 박히거나 오히려 느려집니다(GameFAQs 공략).", en: "Never use Turbo on a corner — save it for the final straightaway. Boosting into a turn just slams you into the wall or slows you down (per GameFAQs guide)." },
        { ko: "상대 차는 아무리 앞서 나가도 거의 항상 「고무줄(rubber band)」처럼 따라붙으므로, 초반에 무리해서 벌리기보다 막판 직선 터보로 승부를 보는 편이 안정적입니다.", en: "The rival always rubber-bands back no matter how far ahead you get, so don't burn yourself out building an early lead — settle it with the final-straight Turbo instead." },
        { ko: "본격 레이스(예: Flight of Fancy)에 들어가기 전에 택시 포인트로 차를 충분히 튜닝해 두세요. 운송·대화 미션을 먼저 돌면 시작 포인트가 넉넉합니다(Steam 대화 미션 가이드).", en: "Tune your car well with Taxi Points before the actual races (e.g. Flight of Fancy); doing the delivery and conversation missions first gives you a healthy starting pile (Steam conversation-mission guide)." },
      ],
      videos: [
    ...
    },
    {
      ...
      howTo: [
        ...
        { ko: "곰은 좌우로 어슬렁대는 방어 자세일 때는 조준을 미리 읽고 피해버립니다. 「돌진(charge) 상태」나 뒷발로 일어선 순간 머리를 노려 쏘세요. 돌진 중에는 시선이 땅을 향해 총구 움직임을 추적하지 못합니다(Steam·Nintendo Okie 가이드).", en: "A bear in its defensive side-to-side pacing reads your aim and dodges — only shoot for the head during its charge or when it rears up on its hind legs. Mid-charge it stares at the ground and can no longer track your rifle (Steam & Nintendo Okie guides)." },
        { ko: "조준 속도가 빠른 「Vertical Double Rifle」을 쓰면 움직이는 표적을 훨씬 안정적으로 맞힙니다. 사슴 무리에서는 뿔 달린 수사슴만 반격하므로 그 녀석부터 헤드샷으로 처리하세요.", en: "The Vertical Double Rifle aims faster than the horizontal one, steadying shots on moving targets. In a deer herd only the antlered buck fights back — drop it first with a headshot." },
        { ko: "함정은 기본적으로 효율이 낮아 거의 의뢰(마텐 포획)용으로만 쓰입니다. 마텐은 「케이지 트랩」 2~3개를 흰 빛(설치 가능 지점)에 깔아 잡고, 사냥감 근처에 가면 음악이 흘러 위치를 알 수 있습니다. 의뢰 5건도 컴플리션 대상입니다(TrueAchievements).", en: "Traps are mostly inefficient and only really used for contracts (catching martens): lay 2–3 cage traps on the white-light spots, and listen for the music cue that plays when you're near huntable game. The 5 contract missions also count toward completion (TrueAchievements)." },
      ],
      videos: [
    ...
    },
    {
      ...
      howTo: [
        ...
        { ko: "정확 입력을 쌓으면 댄스 게이지가 마커를 넘고, 그때 「폭탄(bomb)」 카운트다운이 돌지 않는 타이밍에 L1을 눌러 댄스 히트 무브를 발동합니다. 점수기로 단번에 격차를 벌리세요(GameFAQs).", en: "Stacking accurate inputs pushes the dance gauge past its marker; press L1 (while the 'bomb' isn't counting down) to fire a Dance Heat move and open up a scoring lead in one shot (GameFAQs)." },
        { ko: "버거우면 난이도를 낮춰도 트로피·컴플리션에는 전혀 지장이 없으니 부담 없이 조정하세요(루리웹 트로피 가이드).", en: "If it's tough, lowering the difficulty has zero impact on the trophy or completion — adjust it freely (Ruliweb trophy guide)." },
      ],
      videos: [
    ...
    },
    {
      ...
      howTo: [
        ...
        { ko: "댄스 배틀은 그냥 「이기는」 게 아니라 「살아남으면서 이기는」 게임입니다. 하루카는 시작부터 소량 회복기 「Heal Me Mix」를 가지고 있으니, HP가 위험하면 점수기 대신 회복기를, 여유로우면 「Rainbow Star」 계열 점수기를 L1로 발동해 운영하세요(Yimbocarimbo·GameFAQs).", en: "Dance battles are about surviving AND winning, not just winning — Haruka starts with the small heal 'Heal Me Mix', so juggle: pop a heal with L1 when HP gets low, and a 'Rainbow Star' scoring move when you're safe (Yimbocarimbo & GameFAQs)." },
        { ko: "히트 무브는 △□○×에 직접 배정합니다. 리듬에 약하다면 Body 업그레이드의 「Heart-X」 무브를 배정해 상대 HP를 빠르게 깎아 조기 종료하는 빌드가 가장 안정적입니다.", en: "You assign Heat moves to the △□○× face buttons. If your rhythm is shaky, the most reliable build is to slot Body-upgrade 'Heart-X' moves, which drain the opponent's HP fast to end the battle early." },
        { ko: "스토리를 모두 깨면 이와오 다리 밑에서 요코(Yoko) 수행이 열립니다. 두 번이면 클라이맥스 게이지가 가득 차지만, 끝까지 모두 진행하면 최종 보상으로 「클라이맥스 게이지 무한」을 얻으니 반드시 끝까지 하세요. 프린세스 리그 T-Set전은 댄스 간주 직후 클라이맥스 히트를 꽂는 게 정석입니다(Yimbocarimbo).", en: "After clearing the stories, Yoko's training opens under Iwao Bridge. Two sessions fill the Climax Gauge, but finish ALL of them — the final reward is an infinite Climax Heat Gauge. Against T-Set in the finals, the standard line is to fire Climax Heat right after the dance interlude (Yimbocarimbo)." },
        { ko: "트로피·컴플리션 주의: 프린세스 리그 도중 「사퇴」 선택지를 고르면 트로피 획득이 불가능해집니다. 댄스 배틀은 난이도를 낮춰도 트로피에 영향이 없으니 부담되면 난이도를 내려도 됩니다(루리웹 트로피 가이드).", en: "Trophy/completion warning: picking the 'retire' option mid-Princess League makes the trophy unobtainable. Lowering the dance-battle difficulty does NOT affect the trophy, so drop it if you're struggling (Ruliweb trophy guide)." },
      ],
      videos: [
        { title: { ko: "프린세스 리그 결승 하루카 vs T-Set (Yakuza 5 Remastered)", en: "Princess League Finals — Haruka vs T-Set (Yakuza 5 Remastered)" }, url: YT("7RAFn90Uj6k") },
      ],
    ...
    },
    {
      ...
      howTo: [
        ...
        { ko: "이가라시 코치는 고시키 거리 동쪽 「FIRST SHOT」 카운터에 있습니다. 첫 수행을 깨면 히트 아이가 해금되고, 이후 「히트 아이 무한」 코스에서 L1을 무제한으로 써서 슬로우 상태로 노란색 타이밍을 마음껏 맞힐 수 있습니다(루리웹·Yimbocarimbo).", en: "Find Coach Igarashi at the 'FIRST SHOT' counter on the east side of Goshiki Street; clearing his first lesson unlocks Heat Eye, and afterward an 'unlimited Heat Eye' course lets you spam L1 to nail the yellow timing in slow-mo all you want (Ruliweb & Yimbocarimbo)." },
        { ko: "점수는 파울 10점, 안타 50점, 홈런 100점입니다. 컴플리션은 6개 코스를 전부 깨는 것이고, 「날려버린 남자」 트로피는 의뢰 미션 10건을 따로 요구하므로 챌린지와 의뢰를 함께 챙기세요(GameFAQs·루리웹).", en: "Scoring is 10 for a foul, 50 for a hit, 100 for a home run. Completion means clearing all 6 courses, and the 'home-run king' trophy separately needs 10 commission missions — knock out the challenges and contracts together (GameFAQs & Ruliweb)." },
      ],
      achievementSlug: "achievement_42",
    },
    {
