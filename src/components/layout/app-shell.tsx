      ...
      <aside className="shell-sidebar">
        ...
        <div className="mb-2 rounded-xl border border-[var(--border-subtle)] bg-[var(--bg-elevated)] px-3.5 py-3">
          <div className="mb-1.5 flex items-center gap-1.5">
            ...
            <span className="text-[14px] font-bold uppercase tracking-wider text-[var(--accent)]">PRO</span>
          </div>
          <div className="text-[14px] leading-snug text-[var(--text-secondary)]">{m.misc.proHint}</div>
          <div className="mt-1 text-[14px] text-[var(--text-tertiary)]">{m.misc.proCta}</div>
        </div>
        ...
        <div className="flex items-center gap-3 rounded-xl border border-[var(--border-subtle)] bg-[var(--bg-elevated)] px-3 py-2.5">
          ...
          <div className="min-w-0 flex-1">
            <div className="truncate text-[14px] font-semibold text-[var(--text-primary)]">{user.name}</div>
            <div className="font-mono text-[14px] text-[var(--text-tertiary)]">{user.status}</div>
          </div>
      ...
      <nav className="shell-tabbar">
        {NAV_ITEMS.map(({ id, href, Icon }) => {
          ...
          return (
            ...
            >
              <Icon size={22} />
              <span className={cn("text-[14px]", active && "font-semibold")}>{m.nav[id]}</span>
            </Link>
          );
        })}
      </nav>
