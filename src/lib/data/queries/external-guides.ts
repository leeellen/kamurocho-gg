...
export type EmptyLotGuide = {
  ...
  /**
   * Direct link to a community 100% / game-clear save file for this title
   * (Steam Community guide or Nexus Mods page). Null when none is curated.
   */
  save: { source: SaveFileSource; url: string } | null;
};
...
export const getEmptyLotGuides = cache((locale: Locale): EmptyLotGuide[] =>
  CURATED_GAMES.filter((game) => EXTERNAL_GUIDES[game.slug]?.length)
    ...
    .map((game) => ({
      ...
      save: SAVE_FILES[game.slug] ?? null,
    })),
);
